...
export default function TopBar({ onToggleMobileNav }) {
  ...
  return (
    <header className="h-14 bg-white/95 dark:bg-[#12182B]/95 backdrop-blur-md border-b border-slate-200 dark:border-[#1E294B] flex items-center justify-between px-3 sm:px-6 shrink-0 z-10 select-none w-full max-w-full overflow-hidden">
      {/* Left: Mobile hamburger + Breadcrumbs and Engine Status */}
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        <button
          type="button"
          onClick={onToggleMobileNav}
          className="lg:hidden p-1.5 rounded-lg text-slate-500 hover:text-slate-900 dark:text-[#94A3B8] dark:hover:text-[#F8FAFC] hover:bg-slate-100 dark:hover:bg-[#171F36] shrink-0"
          aria-label="Toggle navigation"
        >
          <span className="material-symbols-outlined text-[20px]">menu</span>
        </button>

        <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-[#94A3B8] min-w-0">
          <Link to="/dashboard" className="hidden sm:inline hover:text-slate-900 dark:hover:text-[#F8FAFC] shrink-0">
            Workspace
          </Link>
          <span className="hidden sm:inline text-slate-400 dark:text-[#64748B]">/</span>
          <span className="text-slate-900 dark:text-[#F8FAFC] font-medium truncate max-w-[130px] sm:max-w-[200px] md:max-w-none">
        ...
        <div className="hidden md:flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-[#171F36] border border-slate-200 dark:border-[#1E294B] text-[11px] text-slate-600 dark:text-[#94A3B8] shrink-0">
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        <Link to="/analyze" className="shrink-0">
        ...
        <Link 
          ...
          title={`Profile: ${user?.email || 'Active Operator'}`}
        >
          {userInitials}
        </Link>
      </div>
